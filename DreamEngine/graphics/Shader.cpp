...
Shader::~Shader()
{
    //no need to delete anything since it's stack object
}
...
Shader load_shader(std::string vertexFile, std::string fragmentFile)
{
	...
	catch (std::ifstream::failure &e)
	{
		...
		std::string error_msg = "ERROR::SHADER::FILE_NOT_SUCCESFULLY_READ " + vertexFile + "," + fragmentFile;
	...
	glShaderSource(vertex, 1, &vShaderCode, nullptr);	// loading
	...
	if (!success)
	{
		...
		throw std::runtime_error("SHADER::FRAGMENT: compilation failed");
	}
